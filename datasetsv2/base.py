...
class BaseDataset(Dataset):
    ...
    def process_pairs(self, ref_image, ref_mask, tar_image, tar_mask, max_ratio=0.9, mkdata=False):
        ...
        inputs:
            ref_image: (H, W, 3)
            ref_mask: [(H, W), (H, W)] or [(H, W)] or (H, W)
            tar_image: (H, W, 3)
            tar_mask: [(H, W), (H, W)] or [(H, W)] or (H, W)
        ...
        if isinstance(ref_mask, np.ndarray) or len(ref_mask) == 1:
            ...
            null_ref = np.zeros_like(item['ref'])
            item['ref'] = np.stack([item['ref'], null_ref], axis=0)
            return item
    ...
    def process_single_pair(self, ref_image, ref_mask, tar_image, tar_mask, max_ratio = 0.8):

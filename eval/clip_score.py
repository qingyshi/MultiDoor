...
parser.add_argument('--clip-model', type=str, 
                    default='/data00/sqy/checkpoints/openai/clip-vit-large-patch14',
...
parser.add_argument('--fake_flag', type=str, default='txt',
                    help=('The modality of real path. '
                          'Default to txt'))
parser.add_argument('--test_dataset_json', type=str, 
                    default='/data00/multidoor_dataset/test_dataset.json',
                    help=('Paths to the json file.'))
...
def calculate_clip_score(dataloader, model, device):
    ...
    for batch in tqdm(dataloader):
        if batch['real'].dim() == 3:  # Check if input is text (input_ids)
            input_ids = batch['real'].squeeze(1)
            real_features = model.get_text_features(input_ids=input_ids.to(device))
        else:
...
def main():
    ...
    dataset = CustomDataset(args.test_dataset_json, processor=processor)
